namespace Nereid
{
	namespace FinalFrontier
	{
		public abstract class Action : Activity
		{
			...
			{
				if (fromVessel != null) {
					var atmosphere = fromVessel.IsInAtmosphere();
					var oxygen = fromVessel.IsInAtmosphereWithOxygen();
					...
					if (atmosphere && oxygen) return ActionPool.ACTION_EVA_OXYGEN;

					if (atmosphere && !oxygen) return ActionPool.ACTION_EVA_INATM;

					if (!atmosphere) return ActionPool.ACTION_EVA_NOATM;
		...
		public class ScienceAction : Action
		{
			...
			{
				try {
					var science = double.Parse(entry.Data);
