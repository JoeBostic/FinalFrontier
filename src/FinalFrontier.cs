...
namespace Nereid
{
	namespace FinalFrontier
	{
		...
		public class FinalFrontier : MonoBehaviour
		{
				...
				if (configuration.UseStockToolbar()) {
					...
					if (ApplicationLauncher.Ready && stockToolbarButton == null) {
						...
						stockToolbarButton = ApplicationLauncher.Instance.AddModApplication(
							...
							DummyVoid,
							DummyVoid,
							ApplicationLauncher.AppScenes.FLIGHT | ApplicationLauncher.AppScenes.SPACECENTER | ApplicationLauncher.AppScenes.MAPVIEW | ApplicationLauncher.AppScenes.SPH | ApplicationLauncher.AppScenes.VAB |
							ApplicationLauncher.AppScenes.TRACKSTATION,
							GameDatabase.Instance.GetTexture(RESOURCE_PATH + "ToolbarIcon", false));
			...
			{
				createBrowserOnce();
				if (browser != null) browser.SetVisible(true);
			}

			private void OnAppLaunchToggleOff()
			{
				if (browser != null) browser.SetVisible(false);
			}
			...
			public void Update()
			{
				var keyAlt = Input.GetKey(KeyCode.LeftAlt);
				var keyCtrl = Input.GetKey(KeyCode.LeftControl);

				var hotkey = Input.GetKeyDown(configuration.hotkey);
			...
			private void AddBlizzysToolbarButtons()
			{
				...
				var iconOn = RESOURCE_PATH + "IconOn_24";
				var iconOff = RESOURCE_PATH + "IconOff_24";
			...
			{
				if (stockToolbarButton != null) stockToolbarButton.toggleButton.Value = false;
			}
