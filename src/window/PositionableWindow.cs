...
namespace Nereid
{
	namespace FinalFrontier
	{
		public abstract class PositionableWindow : AbstractWindow
		{
			...
			{
				if (FinalFrontier.configuration != null) FinalFrontier.configuration.ResetWindowPositions();
				foreach (var window in INSTANCES) window.Reset();
			}

			protected void Reset()
			{
				opened = IsVisible();
				if (FinalFrontier.configuration != null) SetPosition(FinalFrontier.configuration.GetWindowPosition(this));
			}
			...
			protected override void OnOpen()
			{
				...
				if (!opened) {
					if (FinalFrontier.configuration != null) {
						...
						var position = FinalFrontier.configuration.GetWindowPosition(GetWindowId());
						if (position != null)
							SetPosition(position.first, position.second);
						else
