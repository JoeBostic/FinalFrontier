...
namespace Nereid
{
	namespace FinalFrontier
	{
		public abstract class AbstractWindow
		{
			...
			private OnWindowClose onWindowClose;
			...
			protected void UseLeftMouseButtonEvent()
			{
				...
				if (e != null && e.type != EventType.used) {
					if ((e.type == EventType.MouseDown || e.type == EventType.MouseUp) && e.button == 0) e.Use();
					if (e.type == EventType.ContextClick && e.button == 0) e.Use();
			...
			private void CheckBounds()
			{
				...
				// Auto Center
				if (bounds.x == CENTER_HORIZONTALLY) bounds.x = (int) ((Screen.width - bounds.width) / 2);
				if (bounds.y == CENTER_VERTICALLY) bounds.y = (int) ((Screen.height - bounds.height) / 2);
			...
			protected void DrawTooltip()
			{
				...
				if (tooltip != null && tooltip.Trim().Length > 0) {
					var size = FFStyles.STYLE_TOOLTIP.CalcSize(new GUIContent(tooltip));
					var x = mousePosition.x + size.x > bounds.width ? bounds.width - size.x : mousePosition.x;
					var y = mousePosition.y + 32;
					if (x < 0) x = 0;
					if (y < 0) y = 0;

					GUI.Box(new Rect(x - 2, y, size.x + 4, size.y), GUIContent.none);
			...
			{
				if (!visible) return;
				var x = (Screen.width - (int) bounds.width) / 2;
				var y = (Screen.height - (int) bounds.height) / 2;
				SetPosition(x, y);
			}
		}
	}
