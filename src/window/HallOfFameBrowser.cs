...
namespace Nereid
{
	namespace FinalFrontier
	{
		public class HallOfFameBrowser : PositionableWindow
		{
			...
			// Filter
			private HallOfFameFilter filter;

			private readonly RibbonBrowser ribbonBrowser;

			private Vector2 scrollPosition = Vector2.zero;

			// Sorter
			private HallOfFameSorter sorter;
			private readonly GUIStyle STYLE_KERBAL_AREA;
			private readonly GUIStyle STYLE_KERBAL_AREA_EXPANDED;
			private readonly GUIStyle STYLE_KERBAL_BUTTON;
			private readonly GUIStyle STYLE_KERBAL_INFO;
			private readonly GUIStyle STYLE_KERBAL_STATUS;
			private readonly GUIStyle STYLE_RIBBON_AREA;
			...
			protected override void OnWindow(int id)
			{
				...
				if (filter == null || filter.GetScene() != HighLogic.LoadedScene) filter = FinalFrontier.configuration.GetDisplayFilterForSzene(HighLogic.LoadedScene);
				//
				// persistent sorter for displaying kerbals
				if (sorter == null || sorter.GetScene() != HighLogic.LoadedScene) {
					// sorter has changed
					sorter = FinalFrontier.configuration.GetHallOfFameSorterForScene(HighLogic.LoadedScene);
				...
				var index = 0;
				var expandDetected = false;
				var autoexpandEnabled = FinalFrontier.configuration.IsAutoExpandEnabled();
				lock (HallOfFame.Instance()) {
					foreach (HallOfFameEntry entry in HallOfFame.Instance()) {
						// autoexpand this entry on mouse hover?
						var expandedEntry = autoexpandEnabled && expandedRibbonAreaIndex == index && entry.GetRibbons().Count > Constants.MAX_RIBBONS_PER_AREA;
						//
						var kerbal = entry.GetKerbal();
						var info = GetInfo(entry);
						var missionTimeInDays = Utils.GameTimeInDaysAsString(entry.TotalMissionTime) + (GameUtils.IsKerbinTimeEnabled() ? " kerbin" : "");
						if (kerbal != null && filter.Accept(entry) && kerbal.IsCrew()) {
							var buttonTooltip = kerbal.name + ": " + entry.MissionsFlown + " missions, " + missionTimeInDays + " days mission time";
				...
				if (Event.current.type == EventType.Repaint && !expandDetected) expandedRibbonAreaIndex = -1;
			...
			private string GetInfo(HallOfFameEntry entry)
			{
				switch (sorter.GetMode()) {
					...
					case HallOfFameSorter.SORT_MODE.SKILL:
						...
						var specialist = kerbal.experienceTrait.TypeName;
			...
			{
				if (GUILayout.Button(sorter.ModeAsString(), FFStyles.STYLE_NARROW_BUTTON)) sorter.NextMode();
				...
				if (GUILayout.Button(sorter.DirectionAsString(), FFStyles.STYLE_NARROW_BUTTON)) sorter.NextDirection();
			}


			private void DrawStatus(ProtoCrewMember kerbal)
			{
				var status = kerbal.rosterStatus;
				string tooltip;
				switch (status) {
			...
			{
				var x = bounds.x + bounds.width + 8;
				var y = bounds.y;
				if (x + RibbonBrowser.WIDTH > Screen.width) x = Screen.width - RibbonBrowser.WIDTH;
			...
			{
				var rect = new Rect(x, y, ribbon.GetWidth() / scale, ribbon.GetHeight() / scale);
			...
			protected override void OnOpen()
			{
				...
				if (toolbarButton != null) toolbarButton.TexturePath = toolbarButtonTextureOn;
			}

			protected override void OnClose()
			{
				base.OnClose();
				if (toolbarButton != null) toolbarButton.TexturePath = toolbarButtonTextureOff;
			}
			...
			public void registerToolbarButton(IButton button, string textureOn, string textureOff)
			{
				...
				toolbarButtonTextureOn = textureOn;
				toolbarButtonTextureOff = textureOff;
			}
			...
			public class GameSceneBased
			{
				...
				public override bool Equals(object right)
				{
					...
					var cmp = right as GameSceneBased;
			...
			public class HallOfFameSorter : GameSceneBased, Sorter<HallOfFameEntry>
			{
					...
					: base(scene)
				{
					...
					this.mode = mode;
					stats_sorting = stats_sort;
					skill_sorting = skill_sort;
				}
				...
				private void SortBySkill(List<HallOfFameEntry> list)
				{
					list.Sort(delegate(HallOfFameEntry left, HallOfFameEntry right) {
						var sign = direction == DIRECTION.ASCENDING ? 1 : -1;
						var leftkerbal = left.GetKerbal();
						var rightkerbal = right.GetKerbal();
