...
namespace Nereid
{
	...
	{
		internal class ConfigWindow : PositionableWindow
		{
			private readonly CodeBrowser codeBrowser = new CodeBrowser();

			private bool hotkeyInput;

			private readonly GUIStyle STYLE_TEXTFIELD_WIDOWTITLE = new GUIStyle(HighLogic.Skin.textField);
			...
			protected override void OnWindow(int id)
			{
				...
				if (GUILayout.Button("Reset Window Positions", FFStyles.STYLE_BUTTON)) ResetAllWindowPositions();
				// Window Titles
				GUILayout.BeginHorizontal();
				GUILayout.Label("Hall Of Fame window title:", FFStyles.STYLE_STRETCHEDLABEL);
				var hallOfFameWindowTitle = FinalFrontier.configuration.GetHallOfFameWindowTitle();
				...
				var decorationBoardWindowTitle = FinalFrontier.configuration.GetDecorationBoardWindowTitle();
				...
				// Stock Toolbar
				if (ToolbarManager.ToolbarAvailable) config.SetUseStockToolbar(GUILayout.Toggle(config.UseStockToolbar(), "Use Stock Toolbar (needs a restart to take effect)", FFStyles.STYLE_TOGGLE));
				// FAR Calculations
				if (FinalFrontier.farAdapter.IsInstalled()) config.UseFARCalculations = GUILayout.Toggle(config.UseFARCalculations, "Use FAR calculations", FFStyles.STYLE_TOGGLE);
				// Convert Games
			...
			{
				var text = hotkeyInput ? "press key" : FinalFrontier.configuration.hotkey.ToString();
				GUILayout.Label("Hotkey: LEFT-ALT + ");
				if (GUILayout.Button(" " + text + " ", FFStyles.STYLE_BUTTON_HOTYKEY)) hotkeyInput = true;
				if (hotkeyInput)
					if (Input.anyKeyDown) {
						hotkeyInput = false;
						var keys = Utils.GetPressedKeys();
						if (keys != null && keys.Length > 0) FinalFrontier.configuration.hotkey = keys[0];
					}
			}
